...
export function getFoundryContracts() {
  try {
    ...
    return Object.entries(stored)
      .filter(([key, data]) => data.source === "foundry" || key.startsWith("foundry:"))
      .map(([key, data]) => ({
        key,
...
export function saveFoundryConfig(config) {
  try {
    localStorage.setItem("furnacescout_foundry_config", JSON.stringify({
      config,
      timestamp: Date.now(),
    }));
  } catch (error) {
  ...
  try {
    ...
    const filtered = Object.fromEntries(
      Object.entries(stored).filter(
        ([key, data]) => data.source !== "foundry" && !key.startsWith("foundry:")
      )
    );
...
export function getFoundryStats() {
  ...
  return {
    ...
    totalFunctions: contracts.reduce(
      (sum, c) => sum + (c.abi?.filter((item) => item.type === "function").length || 0),
      0
    ),
    totalEvents: contracts.reduce(
      (sum, c) => sum + (c.abi?.filter((item) => item.type === "event").length || 0),
      0
    ),
  };
}
