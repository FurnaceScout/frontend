 ...
 * Fetch and filter token transfers with optimized batch fetching
 */
...
import { parseTokenTransfers } from "./tokens";
...
export async function getTokenTransfers(address = null, options = {}) {
  ...
  try {
    const latestBlock = toBlock || (await getLatestBlockNumber());
    const startBlock = BigInt(fromBlock);
    const endBlock = BigInt(latestBlock);

    const allTransfers = [];

    // Process in chunks to avoid memory issues with large ranges
    const chunkSize = 100n;

    for (
      let currentBlock = startBlock;
      currentBlock <= endBlock && allTransfers.length < limit;
      ...
      // Fetch blocks in batches (optimized)
      const blocks = await fetchBlocksBatched(currentBlock, chunkEnd, {
        includeTransactions: true,
      });

      // Collect all transaction hashes from blocks
      const txHashesWithContext = [];
      for (const block of blocks) {
        if (!block || !block.transactions) continue;

        for (const tx of block.transactions) {
          const hash = typeof tx === "string" ? tx : tx.hash;
          txHashesWithContext.push({
            hash,
      ...
      // Fetch all receipts in batches (optimized)
      const hashes = txHashesWithContext.map((t) => t.hash);
      const receipts = await fetchReceiptsBatched(hashes);
      ...
      // Process receipts and extract transfers
      for (const txContext of txHashesWithContext) {
        const receipt = receiptMap.get(txContext.hash.toLowerCase());
        ...
        for (const transfer of transfers) {
          ...
          allTransfers.push({
            ...
            blockNumber: txContext.blockNumber.toString(),
...
export async function getRecentTokenTransfers(options = {}) {
  ...
  try {
    ...
    const blocks = await fetchBlocksBatched(actualFromBlock, latestBlock, {
      includeTransactions: true,
    });

    // Collect all transaction hashes with their block context
    const txHashesWithContext = [];
    for (const block of blocks) {
      if (!block || !block.transactions) continue;
...
export function filterTransfers(transfers, filters) {
  return transfers.filter((transfer) => {
    ...
    if (
      filters.fromBlock &&
      BigInt(transfer.blockNumber) < BigInt(filters.fromBlock)
    ) {
      return false;
    }
    if (
      filters.toBlock &&
      BigInt(transfer.blockNumber) > BigInt(filters.toBlock)
    ) {
      return false;
    }
