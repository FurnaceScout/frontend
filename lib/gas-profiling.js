 ...
 */
export async function analyzeContractGasUsage(contractAddress, blockRange = 100) {
  try {
    ...
    for (let i = startBlock; i <= latestBlock; i++) {
      ...
      for (const tx of block.transactions) {
        ...
        try {
          ...
          stats.minGasUsed = gasUsed < stats.minGasUsed ? gasUsed : stats.minGasUsed;
          stats.maxGasUsed = gasUsed > stats.maxGasUsed ? gasUsed : stats.maxGasUsed;
    ...
    functions.sort((a, b) => Number(BigInt(b.totalGasUsed) - BigInt(a.totalGasUsed)));
...
export async function getTopGasConsumers(blockRange = 100, limit = 10) {
  try {
    ...
    const contracts = Object.values(contractStats).map((stats) => ({
      ...
      averageGasPerTx: (stats.totalGasUsed / BigInt(stats.transactionCount)).toString(),
    ...
    contracts.sort((a, b) => Number(BigInt(b.totalGasUsed) - BigInt(a.totalGasUsed)));
 ...
 */
export async function compareContractGasUsage(contract1, contract2, blockRange = 100) {
  try {
...
export async function getGasStatistics(blockRange = 100) {
  try {
    ...
    return {
      ...
      overallUtilization:
        Number((totalGasUsed * 10000n) / totalGasLimit) / 100,
      totalTransactions,
