...
export async function analyzeContractGasUsage(
  ...
  try {
    ...
    // Filter transactions for this contract and collect hashes
    ...
    for (const block of blocks) {
      ...
      for (const tx of block.transactions) {
        if (tx.to?.toLowerCase() === contractAddressLower) {
          relevantTxs.push(tx);
    ...
    const hashes = relevantTxs.map((tx) => tx.hash);
...
export async function getTopGasConsumers(blockRange = 100, limit = 10) {
  try {
    ...
    // Collect all transactions with destinations and create lookup map
    const allTxs = [];
    for (const block of blocks) {
      if (!Array.isArray(block.transactions)) continue;
    ...
    // Fetch all receipts in parallel batches
    const hashes = allTxs.map((tx) => tx.hash);
    const receipts = await fetchReceiptsBatched(hashes, { batchSize: 20 });
...
export async function getGasTrends(blockRange = 200, sampleSize = 20) {
  try {
    ...
    // Calculate which blocks to sample
    const sampleBlockNumbers = [];
    for (let i = 0; i < sampleSize; i++) {
      const blockNum = actualStartBlock + BigInt(i * interval);
      if (blockNum > latestBlock) break;
      sampleBlockNumbers.push(blockNum);
    }

    // Fetch sampled blocks in parallel batches
    const blocks = await Promise.all(
      sampleBlockNumbers.map((blockNum) =>
        fetchBlocksBatched(blockNum, blockNum, {
          includeTransactions: true,
          batchSize: 1,
        }).then((arr) => arr[0]),
      ),
    ...
    // Collect all transaction hashes from sampled blocks
    const allTxHashes = [];
    const txToBlockIndex = new Map();
    ...
    blocks.forEach((block, index) => {
      if (block && Array.isArray(block.transactions)) {
        for (const tx of block.transactions) {
          ...
          txToBlockIndex.set(hash.toLowerCase(), index);
        }
      }
    });

    // Fetch all receipts in parallel batches
    const receipts = await fetchReceiptsBatched(allTxHashes, { batchSize: 20 });
    ...
    // Calculate trends for each sampled block
    const trends = blocks.filter(Boolean).map((block) => {
      let totalGasUsed = 0n;
      ...
      if (Array.isArray(block.transactions)) {
        for (const tx of block.transactions) {
          const hash = typeof tx === "string" ? tx : tx.hash;
          const receipt = receiptMap.get(hash.toLowerCase());
          if (receipt) {
