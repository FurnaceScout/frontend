...
export async function calculateTPS(startBlock, endBlock) {
  try {
    ...
    const [startBlockData, endBlockData] = await Promise.all([
      publicClient.getBlock({ blockNumber: BigInt(startBlock) }),
      publicClient.getBlock({ blockNumber: BigInt(endBlock) }),
    ]);
...
export async function getHourlyTransactionStats(hours = 24) {
  try {
    ...
    // Scan recent blocks backwards
    for (let i = latestBlock; i >= 0n; i--) {
      const block = await publicClient.getBlock({
        blockNumber: i,
        includeTransactions: true,
      });
...
export async function getNetworkHealth() {
  try {
    const [latestBlock, avgBlockTime, recentVolume, activeAddrs, recentTPS] =
      await Promise.all([
        ...
        (async () => {
          ...
          const start = Math.max(0, Number(latest) - 50);
          return calculateTPS(start, Number(latest));
        })(),
      ]);
