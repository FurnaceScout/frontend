 ...
 */
export async function scanFoundryDeployments(
  chainId = null,
...
export function getDeploymentByAddress(address) {
  try {
    ...
    return (
      deployments.find(
        (d) => d.contractAddress?.toLowerCase() === normalized,
      ) || null
    );
  } catch (error) {
...
export function linkDeploymentsToABIs(deployments) {
  ...
  try {
    const abis = JSON.parse(localStorage.getItem("furnacescout_abis") || "{}");
...
export function deleteDeployment(address, chainId) {
  try {
    ...
    const filtered = deployments.filter(
      (d) =>
        !(
          d.contractAddress?.toLowerCase() === normalized &&
          d.chainId === chainId
        ),
    );
