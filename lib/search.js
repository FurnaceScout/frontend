...
export async function searchTransactions(criteria) {
  try {
    ...
    // Scan blocks for matching transactions
    for (
      let i = searchStartBlock;
      i <= searchEndBlock && results.length < limit && scannedBlocks < maxBlocks;
      i++
    ) {
      scannedBlocks++;
 ...
 */
function matchesCriteria(tx, block, criteria) {
  const {
    query,
    methodId,
    minValue,
...
export async function searchBlocks(criteria) {
  try {
    const { startBlock, endBlock, minTxCount = 0, maxTxCount = null, limit = 20 } = criteria;

    const latestBlock = await publicClient.getBlockNumber();
    const searchEndBlock = endBlock ? BigInt(endBlock) : latestBlock;
    const searchStartBlock = startBlock ? BigInt(startBlock) : searchEndBlock - 100n;

    const results = [];

    for (let i = searchStartBlock; i <= searchEndBlock && results.length < limit; i++) {
      const block = await publicClient.getBlock({
        blockNumber: i,
        includeTransactions: true,
      });
...
export function getCommonMethodIds() {
  return [
    { id: "0x00000000", name: "Fallback / Transfer", description: "Plain ETH transfer or fallback" },
    { id: "0xa9059cbb", name: "transfer(address,uint256)", description: "ERC20 transfer" },
    { id: "0x23b872dd", name: "transferFrom(address,address,uint256)", description: "ERC20 transferFrom" },
    { id: "0x095ea7b3", name: "approve(address,uint256)", description: "ERC20 approve" },
    { id: "0x42842e0e", name: "safeTransferFrom(address,address,uint256)", description: "ERC721 transfer" },
    { id: "0xf242432a", name: "safeTransferFrom(address,address,uint256,uint256,bytes)", description: "ERC1155 transfer" },
    { id: "0x40c10f19", name: "mint(address,uint256)", description: "Common mint function" },
    { id: "0x42966c68", name: "burn(uint256)", description: "Common burn function" },
    { id: "0xa22cb465", name: "setApprovalForAll(address,bool)", description: "NFT approval" },
  ];
}
...
export function validateSearchCriteria(criteria) {
  ...
  if (criteria.methodId && !isValidMethodId(criteria.methodId)) {
    errors.push("Invalid method ID format (must be 10 characters starting with 0x)");
  }

  // Validate value range
  if (criteria.minValue && isNaN(parseFloat(criteria.minValue))) {
    errors.push("Invalid minimum value");
  }

  if (criteria.maxValue && isNaN(parseFloat(criteria.maxValue))) {
