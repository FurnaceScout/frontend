...
export const LABEL_COLORS = [
  { id: "red", name: "Red", class: "bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400 border-red-200 dark:border-red-800" },
  { id: "orange", name: "Orange", class: "bg-orange-100 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 border-orange-200 dark:border-orange-800" },
  { id: "yellow", name: "Yellow", class: "bg-yellow-100 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400 border-yellow-200 dark:border-yellow-800" },
  { id: "green", name: "Green", class: "bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800" },
  { id: "blue", name: "Blue", class: "bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800" },
  { id: "purple", name: "Purple", class: "bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 border-purple-200 dark:border-purple-800" },
  { id: "pink", name: "Pink", class: "bg-pink-100 dark:bg-pink-900/20 text-pink-700 dark:text-pink-400 border-pink-200 dark:border-pink-800" },
  { id: "gray", name: "Gray", class: "bg-gray-100 dark:bg-gray-900/20 text-gray-700 dark:text-gray-400 border-gray-200 dark:border-gray-800" },
];
...
export function saveAddressLabel(address, label, color = "blue", note = "") {
  ...
  try {
    ...
    window.dispatchEvent(new CustomEvent("labelsUpdated", { detail: { address: normalized } }));
...
export function saveTransactionNote(txHash, note) {
  ...
  try {
    ...
    // Dispatch event for UI updates
    window.dispatchEvent(new CustomEvent("notesUpdated", { detail: { txHash: normalized } }));
...
export function getLabelsStats() {
  ...
  Object.values(labels).forEach(label => {
    if (colorCounts.hasOwnProperty(label.color)) {
...
export function importLabelsAndNotes(jsonString, merge = false) {
  if (typeof window === "undefined") {
    return { success: false, imported: { labels: 0, notes: 0 }, error: "Not in browser environment" };
  }
  ...
  try {
    ...
    if (!data.labels || !data.notes) {
      return { success: false, imported: { labels: 0, notes: 0 }, error: "Invalid format" };
    }
    ...
    localStorage.setItem(STORAGE_KEYS.ADDRESS_LABELS, JSON.stringify(newLabels));
