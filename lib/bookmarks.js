...
export function addBookmark(hash, label, notes = "") {
  ...
  const existing = bookmarks.find((b) => b.hash.toLowerCase() === hash.toLowerCase());
  if (existing) {
...
export function getBookmarkByHash(hash) {
  const bookmarks = getBookmarks();
  return bookmarks.find((b) => b.hash.toLowerCase() === hash.toLowerCase()) || null;
}
...
export function getBookmarkStats() {
  ...
  return {
    ...
    oldest: bookmarks.length > 0 ? bookmarks[bookmarks.length - 1].createdAt : null,
